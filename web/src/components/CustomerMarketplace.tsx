...
export const CustomerMarketplace: React.FC<CustomerMarketplaceProps> = ({
  ...
  const cities = ['All', 'Jaipur', 'Udaipur', 'Bengaluru', 'Delhi', 'Mumbai'];

  // Distance Calculation & Proximity Sorting (Excludes Expired Salons)
  const filteredSalons = salons
    .filter(s => s.subscription_status !== 'expired')
    .filter(s => {
    ...
    .map(s => {
      if (userLocation && s.latitude && s.longitude) {
        const dist = salonDataService.calculateDistanceKm(
          userLocation.lat,
          userLocation.lng,
          s.latitude,
          s.longitude
    ...
    .filter(s => {
      if (selectedRadius === 'all' || !userLocation || s.distance_km === undefined) return true;
      return s.distance_km <= selectedRadius;
    })
    .sort((a, b) => {
  ...
  const handlePaymentSuccess = async (paymentDetails: {
    ...
    const { appointment, token } = await salonDataService.bookAppointmentAndToken({
      ...
      paymentGateway: 'upi',
      paymentScreenshotUrl: paymentDetails.paymentScreenshotUrl
    });
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row items-center gap-3">
              ...
              {/* Geolocation Button & Radius Select */}
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  type="button"
                  onClick={handleGetLocation}
                  disabled={isLocating}
                  className={`px-3 py-2.5 rounded-2xl text-xs font-bold transition-all flex items-center gap-1.5 shrink-0 ${
                    userLocation
                      ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40'
                      : 'bg-slate-900 text-slate-300 border border-slate-800 hover:border-amber-500/40'
                  }`}
                  title="Detect current GPS location to sort nearby salons"
                >
                  <Navigation className={`w-3.5 h-3.5 ${isLocating ? 'animate-spin' : ''}`} />
                  <span>{isLocating ? 'Locating...' : userLocation ? 'GPS Active' : '📍 Use Current Location'}</span>
                </button>
                ...
                {userLocation && (
                  <select
                    ...
                    className="bg-slate-900 border border-slate-800 text-amber-400 text-xs font-bold rounded-2xl px-3 py-2.5 focus:outline-none focus:border-amber-400"
                  >
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
              <span className="text-[11px] text-slate-500 font-semibold mr-1 shrink-0">Cities:</span>
              {cities.map(city => (
                <button
                  key={city}
                  type="button"
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          {salon.distance_km !== undefined && (
                            <span className="text-amber-400 font-mono text-[10px] font-bold px-2 py-0.5 rounded-md bg-amber-500/15 border border-amber-500/30 flex items-center gap-0.5">
                              📍 {salon.distance_km} km away
                            </span>
                          )}
                        ...
                        <p className="text-xs text-slate-400 flex items-center gap-1 mt-1">
                          ...
                          {salon.address}{salon.pincode ? ` - ${salon.pincode}` : ''}
                        </p>
                      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-emerald-500/40 rounded-3xl p-6 sm:p-8 max-w-sm w-full text-center space-y-4 shadow-2xl">
            <div className={`w-16 h-16 rounded-3xl mx-auto flex items-center justify-center ${
              ...
              {isPendingNotice ? <Clock className="w-8 h-8" /> : <CheckCircle2 className="w-8 h-8" />}
            </div>

            <div>
              <span className={`text-xs uppercase tracking-wider font-bold ${
                isPendingNotice ? 'text-amber-400' : 'text-emerald-400'
              }`}>
                {isPendingNotice ? 'Payment Verification Pending' : 'Booking Confirmed!'}
              </span>
              <h3 className="text-xl font-bold font-serif text-white mt-1">
                {isPendingNotice ? 'Screenshot Submitted to Salon' : 'Live Queue Token Issued'}
              </h3>
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800">
              ...
              {isPendingNotice && (
                <div className="mt-3 p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-[11px] text-amber-300 text-left">
                  Notice: Your token will be activated once the salon verifies your UPI payment screenshot.
                </div>
              )}
            </div>
